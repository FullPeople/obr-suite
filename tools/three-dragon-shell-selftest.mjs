...
const channel = suffix => `com.fullpeople/three-dragon-ante/${suffix}`;
...
let shell;
try {
  await build({input:resolve('extensions/three-dragon-ante/src/game/index.ts'),platform:'node',plugins:[{
    name:'shell-ports',resolveId(id){
      if(id==='@owlbear-rodeo/sdk')return '\0sdk';
      if(id==='./controller')return '\0controller';
      if(id==='../locale')return '\0state';
      if(id==='../asset-base')return '\0asset';
      if(id==='../viewport')return '\0viewport';
    },load(id){
      ...
      if(id==='\0controller')return `const f=globalThis.__tableShell;f.calls.imports++;export class TableController {
        ...
        async clearGesture(){} async start(){f.calls.start++;this.changed(this.view);}async stop(){f.calls.stop++;}async command(command){f.calls.commands.push(command);this.changed(this.view);}
      }`;
    },
  ...
  assert.equal(calls.imports,1);assert.match(calls.open[0].url,/^\/suite\/index.html\?instance=/);
