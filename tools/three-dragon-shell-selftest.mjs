...
const fixture = globalThis.__tableShell = { calls, resizeListeners, sdk: {
  ...
  popover:{async open(value){calls.open.push(value);if(pendingOpen)await pendingOpen;},async close(id){calls.close.push(id);},async setWidth(id,value){calls.widths.push(value);},async setHeight(id,value){calls.heights.push(value);}},
...
const send=(suffix,data={},connectionId='alice-connection')=>{
  const instance=calls.open.at(-1)?.url.split('instance=')[1]??'';
...
try {
  await build({input:resolve('src/modules/threeDragonAnte/index.ts'),platform:'node',plugins:[{
    name:'shell-ports',resolveId(id){
      ...
      if(id==='\0controller')return `const f=globalThis.__tableShell;f.calls.imports++;export class TableController {
        constructor(changed){this.changed=changed;}get view(){return {table:null,selfPlayerId:'alice',isHost:false,connected:true,pending:false,game:null};}
  ...
  send('command',{instance:calls.open[0].url.split('instance=')[1],clientId:'page-one',command:{type:'close'}});
  ...
  assert.equal(calls.widths.at(-1),358);assert.equal(calls.heights.at(-1),656);assert.equal(calls.open.length,2,'Resize preserves the existing iframe');
  ...
  metadata={[channel('table')]:{hostPlayerId:'alice'}};await shell.setupThreeDragonAnte();await until(()=>calls.start===2);assert.equal(calls.open.length,2,'Host recovery stays in background');await shell.teardownThreeDragonAnte();
  metadata={};await shell.setupThreeDragonAnte();let finishOpen;pendingOpen=new Promise(done=>finishOpen=done);send('open');await until(()=>calls.open.length===3);
  const stopping=shell.teardownThreeDragonAnte();finishOpen();await stopping;pendingOpen=undefined;
  assert.equal(calls.close.length,3,'All three opened panels, including the late SDK open, must close');
  assert.equal(resizeListeners.size,0);assert.equal(calls.stop,3);assert.equal(calls.commands.length,1);
