"""Build the reviewed English companion-field increment without saving source cards."""
from pathlib import Path
from datetime import datetime,timezone
import argparse,json
import companion_fields as fields

if not __debug__:raise RuntimeError('Verification requires assertions; do not use -O')
BASE=fields.R.parent/'_audit/xlsx-companions'
...
def run(input2014,input2024,prepare=False,author_path=None,output=None):
    out=fresh(output,'companion-plan-' if prepare else 'companion-candidate-')
    ...
    for version,(raw,report) in products.items():
        path=out/(version+'-COMPANIONS-NO-CACHE-NOT-FOR-UPLOAD.xlsx')
