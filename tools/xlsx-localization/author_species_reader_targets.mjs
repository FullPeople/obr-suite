...
assert.equal(plan.schema, 'obr-species-reader-plan/v1');
...
for (const [col, width] of [['A', 28], ['B', 9], ['C', 24], ['D', 100]]) sheet.getRange(`${col}:${col}`).format.columnWidth = width;
sheet.getRange('A1:D1').format.font.bold = true;
sheet.freezePanes.freezeRows(1);
const inspection = await book.inspect({kind: 'table', range: 'English text!A1:D6', include: 'values,formulas', tableMaxRows: 6, tableMaxCols: 4});
await fs.writeFile(output + '.inspect.ndjson', inspection.ndjson);
const preview = await book.render({sheetName: 'English text', range: 'A1:D6', scale: 1, format: 'png'});
