"""Build the reviewed English lookup-label increment without saving source cards."""
from pathlib import Path
from datetime import datetime,timezone
import argparse,json
import main_lookup_labels as fields

if not __debug__:raise RuntimeError('Verification requires assertions; do not use -O')
BASE=fields.R.parent/'_audit/xlsx-main-lookup-labels'
...
def run(input2014,input2024,prepare=False,author_path=None,output=None):
    out=fresh(output,'lookup-plan-' if prepare else 'lookup-candidate-')
    ...
    for version,(raw,report) in products.items():
        path=out/(version+'-LOOKUP-LABELS-NO-CACHE-NOT-FOR-UPLOAD.xlsx')
