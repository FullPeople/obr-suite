"""Build reviewed English background data without saving source cards."""
from pathlib import Path
from datetime import datetime,timezone
import argparse,json
import background_data_fields as fields

if not __debug__:raise RuntimeError('Verification requires assertions; do not use -O')
BASE=fields.R.parent/'_audit/xlsx-background-data'
...
def fresh(value,prefix):
    ...
    if not out.name.startswith(prefix):raise ValueError('Expected a fresh '+prefix+' directory')
    if out.is_relative_to(fields.R.resolve()):raise ValueError('Output must be outside the source repository')
...
def run(input2014,input2024,prepare=False,author_path=None,output=None):
    out=fresh(output,'background_data-plan-' if prepare else 'background_data-candidate-')
    ...
    for version,(raw,report) in products.items():
        path=out/(version+'-BACKGROUND-DATA-NO-CACHE-NOT-FOR-UPLOAD.xlsx')
