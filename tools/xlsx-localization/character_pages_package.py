"""Build reviewed English support pages without saving source cards."""
from pathlib import Path
from datetime import datetime,timezone
import argparse,json
import character_pages_fields as fields

if not __debug__:raise RuntimeError('Verification requires assertions; do not use -O')
BASE=fields.R.parent/'_audit/xlsx-character-pages'
...
def run(input2014,input2024,prepare=False,author_path=None,output=None):
    out=fresh(output,'character_pages-plan-' if prepare else 'character_pages-candidate-')
    ...
    for version,(raw,report) in products.items():
        path=out/(version+'-CHARACTER-PAGES-NO-CACHE-NOT-FOR-UPLOAD.xlsx')
