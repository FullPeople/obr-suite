"""Build the reviewed English Yes/No increment without saving source cards."""
from pathlib import Path
from datetime import datetime,timezone
import argparse,json
import main_toggle_fields as fields

if not __debug__:raise RuntimeError('Verification requires assertions; do not use -O')
BASE=fields.R.parent/'_audit/xlsx-main-toggles'
...
def run(input2014,input2024,prepare=False,author_path=None,output=None):
    out=fresh(output,'toggle-plan-' if prepare else 'toggle-candidate-')
    ...
    for version,(raw,report) in products.items():
        path=out/(version+'-MAIN-TOGGLES-NO-CACHE-NOT-FOR-UPLOAD.xlsx')
