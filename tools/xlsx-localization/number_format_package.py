"""Build the reviewed English feet-format increment without saving source cards."""
from pathlib import Path
from datetime import datetime,timezone
import argparse,json
import number_format_fields as fields

if not __debug__:raise RuntimeError('Verification requires assertions; do not use -O')
BASE=fields.R.parent/'_audit/xlsx-number-formats'
...
def run(input2014,input2024,prepare=False,author_path=None,output=None):
    out=fresh(output,'format-plan-' if prepare else 'format-candidate-')
    ...
    for version,(raw,report) in products.items():
        path=out/(version+'-NUMBER-FORMATS-NO-CACHE-NOT-FOR-UPLOAD.xlsx')
