"""Build the reviewed English main-field increment without saving source cards."""
from pathlib import Path
from datetime import datetime,timezone
import argparse,json
import main_remaining_fields as fields

if not __debug__:raise RuntimeError('Verification requires assertions; do not use -O')
BASE=fields.R.parent/'_audit/xlsx-main-remaining'
...
def run(input2014,input2024,prepare=False,author_path=None,output=None):
    out=fresh(output,'remaining-plan-' if prepare else 'remaining-candidate-')
    ...
    for version,(raw,report) in products.items():
        path=out/(version+'-MAIN-REMAINING-NO-CACHE-NOT-FOR-UPLOAD.xlsx')
