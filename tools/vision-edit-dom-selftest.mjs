...
try{
 ...
 await build({input:resolve("src/modules/fullFog/dynfog/light/edit-page.ts"),plugins:[{
  ...
  load(id){if(id==="\0test-language")return "export const getLocalLang=()=>window.visionMock.lang;";},
 ...
 for(const lang of ["zh","en"]){
  ...
  await page.addScriptTag({content:js});await page.waitForFunction(()=>document.querySelector("select")?.options.length===5);
  ...
  await page.evaluate(()=>{window.visionMock.holdWrite=false;});await page.locator("select").selectOption("team");
  await page.evaluate(()=>window.visionMock.pending.splice(0).forEach(fn=>fn()));
  check(await page.evaluate(()=>window.visionMock.items.a.metadata["com.obr-suite/fullFog/vision"].mode==="team"),`${lang} delayed older owner choice cannot replace newer choice`);
  await page.locator("select").selectOption("owner:p2");
  await page.evaluate(()=>{window.visionMock.holdWrite=true;});await page.locator("select").selectOption("gm");
