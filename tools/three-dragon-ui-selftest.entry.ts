...
// Exercise the actual production DOM fallback, not a stubbed stage/UI. Keep 2D
// canvas available for textures; only the browser's WebGL capability is absent.
...
w.commands=[];w.base={actionReceiptVersion:1,table,selfPlayerId:"p0",isHost:true,connected:true,pending:false,game:projectSeat(state,"s0")} satisfies TableView;
...
w.scenario=(count:number,ownTurn=false)=>{
 ...
 for(let serial=0;serial<160;serial++){
  if(game.round>=2&&game.stage==="play"&&(!ownTurn||eligibleActions(game,"s0").some(action=>action.kind==="play")))break;
 ...
 const result={actionReceiptVersion:1 as const,table:{...table,seats},selfPlayerId:"p0",isHost:true,connected:true,pending:false,game:projectSeat(game,"s0")};
 w.otherVisualHand=game.seats.slice(1).flatMap(seat=>seat.hand);w.visualPublic=projectPublic(game);w.set(result);return result;
};
w.set=(view:TableView)=>{w.current=view;w.ui.update(view);};
// Explicit fixture rejection closes a deliberately submitted synthetic choice.
// Ordinary view updates never manufacture a receipt or imply acceptance.
w.rejectLast=()=>{const action=w.commands.findLast((command:TableUICommand)=>command.type==="action")?.action;
  if(!action)throw Error("No submitted action to reject");
  w.set({...w.current,actionReceipt:{actionId:action.id,tableId:w.current.table.id,gameId:w.current.game.id,revision:action.revision,ok:false,code:"STALE_REVISION",source:"host"}});
};
