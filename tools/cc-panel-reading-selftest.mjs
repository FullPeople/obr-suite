...
try{
 ...
 const readyEventRecovery=await open({holdReady:true,activeCard:'a'});await readyEventRecovery.waitForFunction(()=>window.panelAudit.readyReads.length===1);await readyEventRecovery.evaluate(()=>window.panelAudit.rejectReady());await tick(readyEventRecovery);await readyEventRecovery.evaluate(()=>window.panelAudit.emit('metadata',window.panelAudit.metadata));await tick(readyEventRecovery);const recoveredByMetadata=await state(readyEventRecovery);findings.push({scenario:'rejected-readiness-recovers-from-valid-metadata-without-ready-event',state:recoveredByMetadata});check('readiness error accepts valid metadata recovery',ids(recoveredByMetadata)==='a,d,p'&&visible(recoveredByMetadata)==='a'&&!recoveredByMetadata.errorText,recoveredByMetadata);await readyEventRecovery.close();
