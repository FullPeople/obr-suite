...
class Turntable {
  ...
  async load(track, autoplay = true) {
    if (roomConnected() && track.url) {
      if (this.bus === "bgm") {
        sendToObr({ type: "bgm-load", url: track.url, name: track.name, loop: !!track.loop, position: 0 });
      } else {
        // Keep this id: in paired mode there is no local deck to fire `ended`,
        // so stop() is the ONLY thing that can sfx-stop this exact sound in the
        // room. Without it the pad never released its slot and the room kept a
        // dead entry while later sounds piled up beside it.
        const sfxId = crypto.randomUUID();
        this._sfxId = sfxId;
        sendToObr({ type: "sfx-add", id: sfxId, url: track.url, name: track.name, loop: !!track.loop });
      }
      return;
    }
...
async function startPairing() {
  // A peer left over from an earlier session can be dead — its signaling
  // socket dropped — while still non-null. Returning silently here is what made
  // 配对 look like it did nothing in a long-lived normal window while a fresh
  // (incognito) profile connected immediately. Rebuild a dead one instead.
  if (_peer && !_peer.destroyed && !_peer.disconnected) return;
  if (_peer) tearDownPair();
  const generation = ++_pairVersion;
  try {
    ...
    // PeerJS drops the signaling socket on idle/long-lived tabs and never
    // recovers by itself. Re-register the SAME code so an OBR client that is
    // dialing it can still find this page.
    _peer.on("disconnected", () => {
      if (generation !== _pairVersion || !_peer) return;
      try { _peer.reconnect(); } catch { tearDownPair(); }
    });
