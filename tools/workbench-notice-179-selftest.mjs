...
const web=process.env.DND_CARD_WEB_ROOT||'D:/Desktop/DND-card-web',out=resolve('workbench-test-output/notices-179');mkdirSync(out,{recursive:true});
...
let documents,saves=0,failSaves=false,failSaveStatus=503,saveDelay=0,previewUrl='';
const server=createServer(async(req,res)=>{const p=decodeURIComponent(new URL(req.url,'http://local').pathname);
 ...
 if(/^\/(characters|api\/character)\//.test(p)){const id=p.split('/').filter(Boolean)[p.startsWith('/api/')?3:2];res.setHeader('Content-Type','application/json');if(req.method==='DELETE'){delete documents[id];res.end(JSON.stringify({ok:true}));return;}if(req.method==='PUT'){if(failSaves){res.writeHead(failSaveStatus);res.end('{}');return;}let body='';for await(const part of req)body+=part;if(saveDelay)await new Promise(r=>setTimeout(r,saveDelay));documents[id]=JSON.parse(body);saves++;res.end(JSON.stringify({ok:true}));}else res.end(JSON.stringify(documents[id]));return;}
...
try{
 ...
 // modal.open is captured by the SDK fixture. Preserve its real per-instance
 // URL when mounting the actual renderer; a bare URL cannot pass READY/ACK.
 const mountToast=async()=>{
  const modal=await bg.evaluate(()=>window.wbMock.effects.filter(e=>e.id==='com.obr-suite/resources/toast-modal').at(-1));
  assert(modal?.url,'notice host must request its modal before renderer mount');
  const href=new URL(modal.url);href.pathname='/toast-test.html';
  assert(href.searchParams.get('noticeInstance'),'real notice instance is required');
  await room.evaluate(src=>{document.querySelector('#toast-test')?.remove();const iframe=document.createElement('iframe');iframe.id='toast-test';iframe.src=src;iframe.style.cssText='width:1000px;height:700px';document.body.append(iframe);},href.href);
  const toast=room.frameLocator('#toast-test');await toast.locator('#stack').waitFor({state:'attached'});return toast;
 };
 ...
 let toast=await mountToast();
 await expect.poll(()=>bg.evaluate(()=>window.wbMock.broadcasts.filter(e=>e.name==='com.obr-suite/resources/toast-ready').length)).toBeGreaterThan(0);
 ...
 // The renderer acknowledges DOM insertion before its deferred Web Audio work.
 // A durable command ACK no longer waits for token projection, so observe the
 // actual renderer's five chime broadcasts with a bounded wait of their own.
 await expect.poll(()=>bg.evaluate(()=>window.wbMock.broadcasts.filter(e=>e.name==='com.obr-suite/sfx'&&e.data.name==='resourceToast').length),{timeout:3000,intervals:[25,50,100]}).toBeGreaterThanOrEqual(5);
 check(true,'original resource chime is broadcast by actual toast renderer');
 ...
 const status={id:'test:notice-condition',name:'通知状态',entry:{id:'test:notice-condition',kind:'condition',name:'通知状态',english:'Notice Condition',source:'CUSTOM',edition:'both',packId:'test',revision:'1',entries:['通知状态正文'],raw:{}},level:1};
 let cond=await command('condition',{itemId:'card:hero',action:'add',condition:status});assert(cond.ok,cond.message);await expect(toast.locator('.toast').filter({hasText:'阿明获得了通知状态'})).toHaveCount(1);check(documents.hero.dnd_card_web.selections.some(s=>s.entry.id===status.id),'condition command persists canonical native state and shows a toast');
 let move=await command('condition',{itemId:'card:hero',action:'transfer',to:'card:second',condition:status});assert(move.ok,move.message);check(!documents.hero.dnd_card_web.selections.some(s=>s.entry.id===status.id)&&documents.second.web_conditions.some(e=>e.id===status.id),'condition transfers native card to legacy card without stock invention');
 let undo=await command('condition',move.result.undo);assert(undo.ok,undo.message);check(documents.hero.dnd_card_web.selections.some(s=>s.entry.id===status.id)&&!documents.second.web_conditions.some(e=>e.id===status.id),'condition history restores two canonical owners');
 await bg.evaluate(()=>{const m=window.wbMock;m.role='PLAYER';m.metadata['com.character-cards/list']=m.metadata['com.character-cards/list'].map(c=>c.id==='second'?{...c,owner_ids:['guest'],visibility:'public',locked:false}:c);m.emit('metadata',m.metadata);m.emit('player',{role:'PLAYER'});});
 const gift=await command('condition',{itemId:'card:hero',action:'transfer',to:'card:second',condition:status});assert(gift.ok,gift.message);check(documents.second.web_conditions.some(e=>e.id===status.id),'player can grant one condition to a visible unlocked non-owned card');
 const denied=await command('condition',{itemId:'card:second',action:'remove',condition:status});check(!denied.ok,'grant does not authorize arbitrary recipient condition removal');
 const giftUndo=await command('condition',gift.result.undo);assert(giftUndo.ok,giftUndo.message);check(!documents.second.web_conditions.some(e=>e.id===status.id),'grant history can undo exactly its own target field');
 await bg.evaluate(()=>{window.wbMock.role='GM';window.wbMock.emit('player',{role:'GM'});});
 let monster=await command('condition',{itemId:'goblin',action:'add',condition:status});assert(monster.ok,monster.message);check(monster.result.snapshots[0].state.conditions.some(c=>c.name===status.name),'monster condition metadata returns a coherent snapshot');
 const hpBefore=documents.hero.core_stats.hp.current;r=await command('stats',{itemId:'card:hero',patch:{health:hpBefore-1},expected:{health:hpBefore}});assert(r.ok,r.message);await expect(toast.locator('.toast').filter({hasText:'生命值'})).toHaveCount(1);check(true,'HP edits produce a real nonblocking notification');
 await bg.evaluate(()=>{const m=window.wbMock;m.items=m.items.map(i=>i.id==='one'?{...i,image:{url:'https://token.invalid/latest.png',width:400,height:600}}:i);m.emit('items',m.items);m.select(['one']);});await expect.poll(()=>received.some(m=>m.type==='selection'&&m.state?.tokenPortrait?.url==='https://token.invalid/latest.png')).toBe(true);check(!documents.hero.dnd_card_web.portrait,'live token image is carried by snapshot without writing a custom portrait');
 ...
 toast=await mountToast();await room.waitForTimeout(150);
 const beforeSfx=await bg.evaluate(()=>window.wbMock.broadcasts.filter(e=>e.name==='com.obr-suite/sfx').length);await bg.evaluate(data=>{const instance=new URL(window.wbMock.effects.filter(e=>e.id==='com.obr-suite/resources/toast-modal').at(-1).url).searchParams.get('noticeInstance');window.wbMock.emit('com.obr-suite/resources/toast-deliver',{connectionId:'connection',data:{id:data.noticeId,instance,data}});},last);await room.waitForTimeout(150);check(await toast.locator('.toast').count()===0&&await bg.evaluate(()=>window.wbMock.broadcasts.filter(e=>e.name==='com.obr-suite/sfx').length)===beforeSfx,'overlay reload deduplicates acknowledged visuals/sounds across iframe instances');
 ...
 // A known HTTP rejection is ordinary failure unless an earlier authoritative
 // write actually committed. A ledger read/revision alone is not a commit.
 await bg.evaluate(()=>{window.wbMock.role='GM';window.wbMock.emit('player',{role:'GM'});});
 failSaveStatus=400;failSaves=true;
 const ordinaryRemove=await command('condition',{itemId:'card:hero',action:'remove',condition:status});
 check(!ordinaryRemove.ok&&!ordinaryRemove.uncertain&&ordinaryRemove.diagnostic.httpStatus===400&&documents.hero.dnd_card_web.selections.some(s=>s.entry.id===status.id),'runtime condition rejection without a stock grant remains a definite failure');
 const ordinarySave=await save(c=>c.selections=c.selections.filter(s=>s.entry.id!==status.id));
 check(!ordinarySave.ok&&!ordinarySave.uncertain&&ordinarySave.diagnostic.httpStatus===400,'ordinary save with a read-only ledger revision does not claim an inventory commit');
 failSaves=false;
 const grantCondition={...status,id:'web:test:ledger-grant',name:'库存授予状态',entry:{...status.entry,id:'test:ledger-grant',name:'库存授予状态',english:'Ledger Grant'}};
 const grantStock={id:'ledger-grant-stock',name:grantCondition.name,kind:'condition',quantity:1,slot:2,revision:1,entry:grantCondition.entry};
 const granted=await issue({action:'add',container:'card:hero',row:grantStock,expected:{'card:hero':inventory().containers['card:hero'].revision}});assert(granted.ok,granted.message);
 await expect.poll(()=>documents.hero.dnd_card_web.selections.some(s=>s.entry.id===grantCondition.entry.id)).toBe(true);
 const revisionBefore=granted.result.inventory.revision;
 failSaves=true;
 const partial=await command('condition',{itemId:'card:hero',action:'remove',condition:grantCondition});
 check(!partial.ok&&partial.uncertain&&partial.diagnostic.code==='CONDITION_GRANT_PARTIAL'&&partial.diagnostic.phase==='save-condition-document'&&partial.diagnostic.itemId==='card:hero'&&partial.diagnostic.ledgerRevision>revisionBefore,'committed grant retirement plus rejected character write reports detailed uncertain partial commit');
 const persistedLedger=JSON.parse(readFileSync(join(process.env.WORKBENCH_DATA_DIR,'inventory_test-room.json'),'utf8'));
 check(!persistedLedger.data.containers['card:hero'].items.some(s=>s.id===grantStock.id)&&documents.hero.dnd_card_web.selections.some(s=>s.entry.id===grantCondition.entry.id),'partial diagnostic reflects actual durable grant removal while character projection remains unchanged');
 failSaves=false;
