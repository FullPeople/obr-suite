"""Verify the six-patch chain and actual corrected parser in a fresh directory.
...
def main():
    ...
    cli.add_argument("--source",type=Path,required=True,help="Unchanged ability-inputs result parser")
    ...
    packages=("sheet-aliases-20260908","shield-input-20260908","spell-aliases-20260908","main-fields-20260909","ability-inputs-20260909")
    ...
    for entry in manifest["workbooks"]:
        ...
        frozen[path]=raw
    require(len(manifest["tests"])==1 and manifest["tests"][0]["file"]=="test_main_remaining.py","Unexpected test list")
    ...
    env={**os.environ,"PYTHONDONTWRITEBYTECODE":"1","OBR_MAIN_REMAINING_PARSER":str(out/"current/parser.py"),
         "OBR_MAIN_REMAINING_BASELINE":str(out/"baseline/parser.py"),"OBR_MAIN_REMAINING_PUBLIC":str(args.xlsx_dir.resolve())}
    command=[sys.executable,"-B","-X","utf8","-W","ignore::DeprecationWarning",str(delivery.HERE/"test_main_remaining.py")]
    ...
    report={"before_sha256":delivery.sha256(before),"after_sha256":delivery.sha256(after),
            "six_patch_chain_apply_reverse_exact":True,"tests_passed":int(count[1]),
