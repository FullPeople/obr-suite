...
await build({ input: resolve("tools/three-dragon-gesture-ui-selftest.entry.ts"), platform: "browser", output: { file: join(out, "app.js"), format: "esm", codeSplitting: false } });
const server = createServer((req, res) => {
  ...
  res.end(url.pathname === "/app.js" ? readFileSync(join(out, "app.js")) : url.pathname === "/style.css" ? readFileSync("extensions/three-dragon-ante/src/game/style.css") : url.pathname === "/client"
...
try {
  ...
  await alice.locator("#hand [data-option]").first().hover();
  ...
  await alice.locator("#hand [data-option]").first().click();
  ...
  mark("Selection lifts only the opponent back; screenshot captures both actual iframe surfaces");
  ...
  mark("Receiver DOM and gesture payload contain no opponent card identities; hover/select issue no rules action or revision");
  // Deselect then leave the entire hand; both ordinal effects must disappear.
  await alice.locator("#hand [data-option]").first().click();
  ...
  mark("Deselect + real pointer exit clears backs and has no idle gesture sends");
  await alice.locator("#hand [data-option]").first().click();
  ...
  assert.equal(await alice.locator("#hand [data-option]").count(), 6);
  mark("Close clears selection on the other UI; reopened UI retains the same actual hand/game");
  await alice.locator("#hand [data-option]").first().click();
  await page.waitForFunction(() => !!document.querySelector("#host").contentDocument.querySelector(".card-back.selected"));
