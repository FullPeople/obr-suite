...
export const pages: readonly { zone: OnboardingZone; eyebrow: Words; title: Words; body: Words; tip: Words; caption: Words; steps?: readonly Words[]; results?: readonly Words[]; glossary: readonly GlossaryTerm[] }[] = [
  { zone: 'hand', eyebrow: ['基本流程', 'Game flow'], title: ['三龙牌', 'Three-Dragon Ante'],
    ...
    glossary: [{ term: ['暗置', 'Face down'], meaning: ['牌面朝下，暂不公开。', 'Keep the card face hidden.'] }, { term: ['轮次', 'Round'], meaning: ['每位玩家各行动一次。', 'Each player takes one turn.'] }, { term: ['轮局', 'Gambit'], meaning: ['牌面也叫斗牌：从暗置到奖池结算，通常三轮。', 'From ante to scoring; usually three rounds. The supplied cards call this 斗牌.'] }] },
  ...
  { zone:'hand', eyebrow:['手牌不足', 'Low hand'], title:['没有牌时，怎么买牌？', 'How do you buy cards?'],
    body:['自己的回合开始时只剩 1 张手牌，或者任何玩家在触发能力全部处理完后没有手牌，就必须买牌。系统自动处理，不需要下注或买牌按钮，也不能随时主动买牌。', 'A purchase is required if you start your turn with only 1 card, or if any player has no hand cards after triggered powers finish resolving. The table handles this automatically; there is no anytime buy or bet button.'],
    results:[['翻开并弃掉牌堆顶 1 张牌，它的力量就是本次价格。', 'Reveal and discard the top card: its strength is the price.'], ['支付该数量的金币，然后从牌堆里抽牌，直到手牌有 4 张。', 'Pay that much gold, then draw from the deck until your hand has 4 cards.']],
    tip:['例如只剩 1 张手牌，翻出的定价牌为 7 点：支付 7 金币，再从牌堆里抽 3 张牌。没有手牌则抽 4 张。金币通常付进赌注堆；贸易亲王的能力生效时，改付给该能力的拥有者。金币不够就先付现有金币，缺额记到轮局末处理。若赌注堆已被取空，轮局立即结束，不再继续处理买牌。', 'Example: with 1 card, revealing a strength-7 price card means paying 7 gold and drawing 3 cards. An empty hand draws 4. Payment normally goes to the stakes; an active Merchant Prince receives it instead. Pay what you have and record any shortfall for the end of the gambit. Empty stakes end the gambit immediately, stopping further purchases.'],
    caption:['定价牌进入弃牌堆，不是你买到的手牌。', 'The price card is discarded; you do not buy that card.'],
    glossary:[{term:['抓牌 / 抽牌', 'Draw'],meaning:['从牌堆顶拿牌加入手牌，上限 10 张。', 'Take cards from the top of the deck, up to a hand limit of 10.']},{term:['赌注堆', 'Stakes'],meaning:['就是中央奖池，不包括前注牌。', 'The central gold pot; ante cards are separate.']}] },
  { zone:'stakes', eyebrow:['轮次与结束条件', 'Rounds and endings'], title:['一轮、轮局、整局有什么区别？', 'Round, gambit, or game?'],
    body:['回合是一个玩家出牌并处理完效果；一轮是所有玩家各完成一次回合；一轮局从大家暗置前注牌开始，通常打三轮再计分；整局由多轮局组成，直到轮局末有人没有金币。牌面上的“斗牌”就是这里的“轮局”。', 'A turn is one player playing and resolving effects. A round gives everyone one turn. A gambit starts with antes and normally lasts three rounds before scoring. A full game contains multiple gambits and ends when someone has no gold after a gambit. On the supplied Chinese cards, 斗牌 means gambit.'],
    results:[['每轮结束：本轮点数最高且不并列的出牌者，下轮先出。全部并列则保留原领出者。', 'After a round: the highest untied card sets the next leader. If every card ties, retain the leader.'], ['第三轮及之后：比较整个牌阵，不是只看最后一张；同分就所有人加打一轮。', 'From round three: compare whole flights, not just the last card. Tied winning totals add a round for everyone.']],
    tip:['德鲁伊可以改成最低总力量获胜，青铜大督军可以要求加轮，龙神条件可以使某位玩家无法赢得轮局。先发奖池，再补付偿债池，最后检查整局是否结束；整局胜者是此时金币最多的人，并获得偿债池金币。若整局还没结束，每人从牌堆里抽 2 张牌，最多持有 10 张，保留已有手牌，再开始新轮局。本牌桌按你提供的牌包执行：湛蓝支配者 8 点、纯白追猎者 10 点、白龙包含 7 点，墨绿策划者要求更强的邪恶龙。', 'Druid can make the lowest total win, Bronze Warlord can add a round, and dragon god restrictions can make a flight ineligible. Award stakes, settle shortfalls into the hole, then check for game end. The richest player wins and receives the hole. Otherwise, everyone draws 2 cards up to 10, keeps their existing hand, and starts another gambit. This table uses the supplied pack: Blue Overlord 8, White Hunter 10, a strength-7 White Dragon, and Green Schemer demanding stronger evil dragons.'],
    caption:['牌面写左侧 = 顺时针相邻玩家；右侧 = 逆时针相邻玩家。', 'On card text, left means the clockwise neighbor; right means the counterclockwise neighbor.'],
    glossary:[{term:['力量 / 点数', 'Strength'],meaning:['卡面数字；计分时要算入特殊效果。', 'The printed number, modified by scoring effects.']},{term:['前注牌', 'Ante'],meaning:['每轮局开始暗置的下注牌。', 'The ante card committed at the start of a gambit.']},{term:['整局胜利', 'Game victory'],meaning:['结束时比金币，不是比赢了几轮局。', 'Most gold at game end, not most gambits won.']}] },
