...
function ensureController(): Promise<void> {
  ...
  starting = (async () => {
    ...
    const instance = new TableController(requestView, { onGesture: (seatId, gesture) => {
      if (active && panelClient && panelOpen) void OBR.broadcast.sendMessage(TABLE_GESTURE,
        { instance: panelInstance, clientId: panelClient, seatId, gesture }, { destination: "LOCAL" }).catch(() => {});
    } });
    controller = instance;
...
export async function setupThreeDragonAnte(): Promise<void> {
  ...
  unsubs.push(OBR.broadcast.onMessage(TABLE_GESTURE, event => {
    ...
    if (active && panelOpen && event.connectionId === connectionId && data?.instance === panelInstance && data.clientId === panelClient && !('seatId' in data))
