...
2026-09-10 04:45 北京时间，Dev **1.0.153-dev** 已上线，修复音乐板房间空间不足及首次配对失败中断网页播放的问题。稳定版仍为 **1.3.0**，尚未包含本次音乐热修。资源路径分别为 `/suite/` 与 `/suite-dev/`，同一房间只启用一个 Suite 通道。

此前 2026-09-09 的双渠道发布修复短休时钟、设置与悬浮栏同步，调整四个默认开关、合并视野归属选项并整理公告；同时发布 Boss、转场/CG、音乐、传送门默认图片和透明展示层修正。历史详情见 [双渠道发布记录](RELEASE_20260909_SUITE_1_3_0.md)。

独立三龙牌 **0.3.2-dev** 同时上线，修复结算内容偶发不可见；保留 0.3.1 的圆桌、暗置区/金币布局、自定义开局和原比例预览。Music Studio 更新为 **20260910-storage**，与 Suite dev 配合验证首次播放配对。三项部署共 473/473 个公网文件核验通过，稳定目录保持原样。当前详情及测试清单见 [运行问题热修记录](DEV_HOTFIX_20260910_SCORING_MUSIC.md)。所有玩家完整刷新，旧桌和存档保持兼容。完整英文 XLSX 与角色卡服务器补丁尚未发布。
