...
15:15 北京时间，稳定版 **1.3.0** 和 Dev **1.0.152-dev** 均已上线。两渠道由同一源码 `6b7e945da11bea4e3421c0b7a81c114c2cfc370c` 独立构建，功能一致，资源路径分别为 `/suite/` 与 `/suite-dev/`。672/672 个公网发布文件核验通过。
...
独立三龙牌 **0.2.2-dev** 已于 18:34 发布，补全中英文卡牌效果、修复公开牌查看、加入三类本地音效，以及 1.5 秒能力展示和等待选择时的来源牌光边；27/27 个公网文件核验通过。两渠道设置都提供此独立测试版地址。[三龙牌发布记录与测试清单](RELEASE_20260909_THREE_DRAGON_0_2_2.md)。此前 [0.2.1-dev](RELEASE_20260909_THREE_DRAGON_0_2_1.md) 的自动教学、原位翻牌和操作修正保留。Music Studio 未再修改。完整英文 XLSX 与角色卡服务器补丁未发布。同一房间只启用一个 Suite 通道。
