...
2026-09-09 15:15 北京时间，稳定版 **1.3.0** 和 Dev **1.0.152-dev** 均已上线。两渠道由同一源码 `6b7e945da11bea4e3421c0b7a81c114c2cfc370c` 独立构建，功能一致，资源路径分别为 `/suite/` 与 `/suite-dev/`。672/672 个公网发布文件核验通过。

本轮修复短休时钟、设置与悬浮栏同步，调整四个默认开关、合并视野归属选项并整理公告。稳定版同时接收此前 Dev 的 Boss、转场/CG、音乐、传送门默认图片和透明展示层修正。详情、产物指纹和测试清单见 [双渠道发布记录](RELEASE_20260909_SUITE_1_3_0.md)。

独立三龙牌 **0.3.1-dev** 已于 2026-09-10 04:00（北京时间）发布：圆形三维牌桌、暗置区固定在牌阵左侧、金币直接压在各自暗置牌上；使用用户提供的钱币图，不加托盘。增加创建者自定义开局参数，悬浮仅放大原比例牌面，玩法介绍改为两页。129/129 个公网文件核验通过。两渠道设置均提供独立测试版地址。[发布记录与快速测试](RELEASE_20260910_THREE_DRAGON_0_3_1.md)。所有玩家完整刷新，0.3.0 旧桌和存档兼容；测试新参数时重新开局。牌包数值、创建者独立开局、联机能力展示和结算过程保留。Music Studio 未修改。完整英文 XLSX 与角色卡服务器补丁未发布。同一房间只启用一个 Suite 通道。

此前 14:38 的 Dev 1.0.151 热修见 [透明层修正记录](DEV_HOTFIX_20260909_TRANSPARENCY.md)，14:01 的功能发布见 [此前发布记录](DEV_RELEASE_20260909_FEEDBACK.md)。它们是历史部署基线。

## 历史说明（2026-08-25，稳定版 1.2.0 发布后）

以下“功能完全一致”和版本号描述仅适用于当时的部署，由上面的当前部署记录取代。
