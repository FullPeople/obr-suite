...
独立三龙牌 **0.3.0-dev** 已于 2026-09-10 01:36（北京时间）发布：使用用户牌包的 100 张正面及对应数值、中文效果，保留单龙矢量卡背；新增轮次横幅与逐张计分过程，补充买牌和整局规则说明。127/127 个公网文件核验通过。两渠道设置均提供此独立测试版地址。[发布记录与快速测试](RELEASE_20260910_THREE_DRAGON_0_3_0.md)。所有玩家完整刷新后创建新桌，旧桌与旧存档不改写。此前创建者独立开局、联机点击关闭能力说明和顺时针座位均保留。Music Studio 未修改。完整英文 XLSX 与角色卡服务器补丁未发布。同一房间只启用一个 Suite 通道。
