...
14:38 北京时间，dev 已更新为 **1.0.151-dev**，修复 Boss/转场展示层的黑色画布；运行源码 `b6b91ac800032ce00271e1688e5e1e067f16eeef`。其余站点未更新，见[热修记录](DEV_HOTFIX_20260909_TRANSPARENCY.md)。以下为当天 14:01 的功能发布基线。

dev 已部署 **1.0.150-dev**，运行源码提交 `f1e7e0faf5be88e78e4795f27525026ceb8e4907`。stable 保持 **1.2.2**，312 个文件未变。dev 包含运行修复、共享视野、透明 Boss 血条、休息转场、后台音乐及本轮反馈修正；共享指针已取消，三龙牌改为独立 **0.1.0-dev** 扩展，由 DM 自行安装。共享 Music Studio 的相容同步修订同时上线。完整英文 XLSX 尚未发布，角色卡服务器补丁未安装。当前等待第二轮真实房间测试。

安装与逐项验收见 [第二轮 dev 测试清单](DEV_TEST_CHECKLIST_20260909_FEEDBACK.md)，部署证据见 [本轮发布记录](DEV_RELEASE_20260909_FEEDBACK.md)。同一房间仍只装一个 Suite 通道，独立三龙牌可与它一起安装。
