...
dev 已部署 **1.0.149-dev**，运行源码提交 `a74f50daa156f4372553ad3df39916af7b0e6dec`。stable 保持 **1.2.2**，本次未部署。两个线上通道现在有功能差异：dev 包含新版运行修复、共享视野、Boss 血条、转场、后台音乐、三龙牌和共享指针；完整英文 XLSX 尚未发布，角色卡服务器补丁未安装。用户要求暂停后续开发，先进行真实房间测试。

安装与逐项验收见 [dev 测试清单](DEV_TEST_CHECKLIST_20260909.md)，部署证据见 [发布记录](DEV_RELEASE_20260909.md)。同一房间仍只装一个通道。
