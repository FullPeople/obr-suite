...
独立三龙牌 **0.2.5-dev** 已于 2026-09-10 00:29（北京时间）发布：修复联机能力展示漏播；教程和卡牌说明改用顺逆时针，三维座位与顺时针出牌一致。27/27 个公网文件核验通过。两渠道设置均提供此独立测试版地址。[发布记录与快速测试](RELEASE_20260910_THREE_DRAGON_0_2_5.md)。此前创建者独立开局、点击关闭能力说明均保留。Music Studio 未修改。完整英文 XLSX 与角色卡服务器补丁未发布。同一房间只启用一个 Suite 通道。
