...
独立三龙牌 **0.2.4-dev** 已于 21:41 发布：普通玩家可创建牌桌，由创建者开局、准备新局，无需 DM 参与；修复创建者另一个在线窗口无法开局的问题。27/27 个公网文件核验通过。两渠道设置均提供此独立测试版地址。[发布记录与快速测试](RELEASE_20260909_THREE_DRAGON_0_2_4.md)。此前 [0.2.3-dev](RELEASE_20260909_THREE_DRAGON_0_2_3.md) 的点击关闭能力说明保留。Music Studio 未修改。完整英文 XLSX 与角色卡服务器补丁未发布。同一房间只启用一个 Suite 通道。
