...
import { createContentRequests } from "../../utils/contentRequests";
// Clicked token details have their own bounded lane; bulk catalog files cannot starve them.
const fetchContentJson=createContentRequests(2);
...
/** Is the scene snapshot already what the loop below would resolve — without
 *  touching the network? A catalog-annotated snapshot records the library,
 *  base, content language and content fingerprint it was loaded from, so
 *  comparing those four against this client's current selection decides the
 *  question locally. Only a mismatch (language flip, a library added or
 *  reordered, an edited snapshot) is worth downloading a whole
 *  `bestiary-*.json` for. */
function snapshotMatchesSelection(shared: any, sources: ContentSource[]): boolean {
  const marker = shared?._suiteContent;
  const primary = sources[0];
  if (!primary || !marker || marker.authored === true) return false;
  if (marker.libraryId !== primary.id || marker.base !== primary.base || marker.language !== primary.language) return false;
  return marker.fingerprint === monsterFingerprint(shared);
}
...
export async function fetchLocalizedMonster(slug: string, shared?: any): Promise<any | null> {
  ...
  // 2026-09-14 — hand the snapshot straight back when it is provably the
  // record this client would end up with. Imported local content still wins
  // unconditionally (it is re-resolved below so a re-imported file reaches
  // already-bound tokens), so the shortcut only covers the remote case.
  if (!local && snapshotMatchesSelection(shared, sources)) return shared;
