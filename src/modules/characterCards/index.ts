...
async function openMainPopover() { if (WORKBENCH_DEV) { await OBR.action.open(); return; } if (!active || !sceneReady) return; desiredMain = true; await syncMainPanel(); }
async function closeMainPopover() { desiredMain = false; await syncMainPanel(); }
async function toggleMainPanel() {
  if (WORKBENCH_DEV) { await OBR.action.open(); return; }
...
export async function setupCharacterCards(): Promise<void> {
  ...
  try {
    if (!WORKBENCH_DEV) await OBR.tool.create({
