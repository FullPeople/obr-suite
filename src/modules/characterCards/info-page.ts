...
// On-token indicators for the two toggles beside 施法关键属性. An OBR token
// image carries no shader or filter field, and EFFECT items are not accepted by
// OBR.scene.items at all — they exist per-client through OBR.scene.local, so
// players would never see them (see statusTracker/effects.ts). The supported
// route is the one every other status already uses: an attached, locked,
// hit-disabled WebM overlay that OBR keeps glued to the token without position
// writes. The status tracker owns those items, so these two write its metadata
// and reuse its shipped `user-*.webm` assets.
const STATUS_BUFFS_KEY="com.obr-suite/status/buffs";
const STATUS_BUFF_ROUNDS_KEY="com.obr-suite/status/buff-rounds";
const CONCENTRATION_BUFF="u_focused";
const INVISIBILITY_BUFF="u_invisible";
const BUFF_TOGGLES:ReadonlyArray<readonly [string,string]>=[["focus",CONCENTRATION_BUFF],["invisible",INVISIBILITY_BUFF]];
function readBuffIds(item:any):string[]{const raw=item?.metadata?.[STATUS_BUFFS_KEY];return Array.isArray(raw)?raw.filter((id:unknown):id is string=>typeof id==="string"):[];}
...
/** Apply one toggle to the bound token's buff list. The status tracker's GM
 *  client turns that metadata into the attached overlay, so this writes
 *  metadata only and works from any permitted client. */
async function setTokenBuff(target:InfoTarget,input:HTMLInputElement,buffId:string,wanted:boolean):Promise<void>{
  const itemId=target.itemId;
  if(!itemId)return;
  input.disabled=true;
  try{
    const items=await OBR.scene.items.getItems([itemId]);
    if(!isCurrent(target)||!items[0]||!canNameItem(items[0]))return;
    const current=readBuffIds(items[0]);
    ...
    await OBR.scene.items.updateItems([itemId],(drafts)=>{
      for(const d of drafts){
        ...
        if(!wanted&&rounds&&typeof rounds==="object"&&!Array.isArray(rounds)&&buffId in rounds){
          const trimmed={...(rounds as Record<string,unknown>)};
          delete trimmed[buffId];
          (d.metadata as any)[STATUS_BUFF_ROUNDS_KEY]=trimmed;
        }
      }
    });
    // Keep the render source truthful, so a later language-only re-render paints
    // the state just written instead of the read that preceded it.
    if(isCurrent(target))target.buffs=next;
  }catch(e){
    console.warn("[character-cards/info] buff toggle failed",e);
    input.checked=!wanted;
  }finally{
    if(isCurrent(target)&&input.isConnected)input.disabled=false;
  }
}
...
interface InfoTarget {generation:number;scene:number;cardId:string;roomId:string;itemId:string|null;ownerIds?:string[];canName?:boolean;buffs?:string[];itemSignature?:string;permissionSignature?:string}
...
/** One chip-styled box per toggle, laid out after 豁免DC and 施法关键属性 like
 *  any other chip but holding a checkbox instead of a value. The whole box is
 *  the label, so clicking anywhere in it toggles. State comes from the bound
 *  token's own buff list, so the status tracker and this panel can never
 *  disagree; read-only for a viewer who may not edit this token. */
function buffToggleMarkup():string{
  const canEdit=!!renderedTarget?.itemId&&renderedTarget.canName===true;
  const active=new Set(renderedTarget?.buffs??[]);
  return BUFF_TOGGLES.map(([key,id])=>{
    const on=active.has(id);
    return `<label class="chip toggle${on?" on":""}" title="${escapeHtml(L(`${key}Toggle`))}">`
      +`<span class="k">${escapeHtml(L(key))}</span>`
      +`<span class="v"><input type="checkbox" data-cc-buff="${id}"${on?" checked":""}${canEdit?"":" disabled"}></span>`
      +`</label>`;
  }).join("");
}

/** Reflect the token's buff list into the mounted toggles. Runs on every item
 *  change, so a buff applied or dropped from the status tracker shows up here
 *  without rebuilding the card. */
function syncBuffToggles(item:any):void{
  const active=new Set(readBuffIds(item));
  for(const input of root.querySelectorAll<HTMLInputElement>("input[data-cc-buff]")){
    const wanted=active.has(input.dataset.ccBuff||"");
...
// Concentration / invisibility toggles. Delegated because render() replaces
// root's children on every card switch.
root.addEventListener("change", (event) => {
  const input=(event.target as HTMLElement|null)?.closest<HTMLInputElement>("input[data-cc-buff]");
  if(!input)return;
  const buffId=input.dataset.ccBuff||"",wanted=input.checked,target=renderedTarget;
  if(!isCurrent(target)||!target.itemId||target.canName!==true){input.checked=!wanted;return;}
  void setTokenBuff(target,input,buffId,wanted);
});
...
OBR.onReady(async () => {
  ...
  subscriptions.push(OBR.broadcast.onMessage(BC_CARD_UPDATED, (ev: any) => {
    ...
    else if(item)syncBuffToggles(item);
  }));
