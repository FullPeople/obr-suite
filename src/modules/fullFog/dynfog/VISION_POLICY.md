...
| --- | --- | --- |
| Bound card with no explicit owner list | The bound token's creator | Yes, when the room's sharing switch is on |
...
The card-list schema has an `owner_ids` field but no writer — the card panel
only cycles public↔dm — so a bound card normally carries no owner list. That
case must not resolve to an empty owner set, which left the automatic choice
dark for the whole room. It now takes the bound token's creator for the
personal view and is shareable, which is what the selector's "自动归属，已共享"
label promises; the party view still needs the room's sharing switch.
...
- `node tools/vision-selftest.mjs --mutations`: 90 assertions pass; all 16
...
- `node tools/vision-edit-dom-selftest.mjs`: 38 assertions pass in headless Edge,
