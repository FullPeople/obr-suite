...
export class StudioPeer {
  ...
  private attempts = 0;
  /** True once ANY connection of this attempt chain has opened. Distinguishes
   *  "this pairing never worked" (give up loudly) from "a working session lost
   *  its network" (keep retrying, playback must survive the gap). */
  ...
  async connect(code: string, restoring = false): Promise<void> {
    this.disconnect(false); this.code = code.trim().toUpperCase(); this.attempts = 0; this.everConnected = false;
  ...
  private async dial(restoring: boolean): Promise<void> {
    ...
    const retry = () => {
      ...
      // A pairing that never once opened must stop instead of dialing a dead
      // Studio registration forever — that endless restore is what made a
      // normal window fail while a fresh profile connected immediately.
      if(!this.everConnected&&this.attempts>=3){this.status="error";this.changed();return;}
    ...
    try {
      ...
      this.peer.on("open", () => {
        ...
        connection.on("open", () => { if (!current()) return; this.attempts = 0; this.everConnected = true; this.status = restoring ? "restored" : "connected"; this.changed(); });
  ...
  disconnect(notify = true): void {
    this.code = ""; this.version++; this.everConnected = false; if (this.retry) clearTimeout(this.retry); this.retry = undefined;
