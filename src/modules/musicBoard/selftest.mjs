...
async function audioTest(mutation) {
  const voices = [], claimed = [];
  class MockAudio extends EventTarget {
    constructor(url) { super(); this.src = url; this.crossOrigin = null; this.currentTime = 0; this.duration = 100; this.loop = false; this.paused = true; this.playCalls = 0; this.loadCalls = 0; voices.push(this); }
    play() { this.playCalls++; this.paused = false; return Promise.resolve(); }
    pause() { this.paused = true; } removeAttribute() { this.src = ""; } load() { this.loadCalls++; }
    /** A host with no Access-Control-Allow-Origin header: the CORS-mode load
     *  fails outright, so the element stays paused and fires `error`. */
    failLoad() { this.paused = true; this.dispatchEvent(new Event("error")); }
  }
  ...
  globalThis.Audio = MockAudio; globalThis.AudioContext = class { state = "running"; currentTime = 0; destination = {}; createGain = gain;
    ...
    createMediaElementSource(audio) { claimed.push(audio); return gain(); } async resume() {} async close() { this.state = "closed"; } };
  const { MusicAudio } = await bundle("audio", mutation), { emptySession, reduceMusic } = await bundle("model"); let ended = 0, sfxFinshed = 0;
  const engine = new MusicAudio(() => {}, () => ended++, () => sfxFinshed++); let state = reduceMusic(emptySession(), { type: "play", track: track("one") }, "play-one");
  ...
  // A direct link whose host sends no ACAO header cannot load in CORS mode,
  // but the track must still play. Asserted on the track the room is playing.
  const corsHost = voices[0], corsUrl = corsHost.src; corsHost.failLoad(); await settle();
  const retried = voices.at(-1);
  assert.notEqual(retried, corsHost, "failed CORS load re-mounts instead of reusing the silenced element");
  assert.equal(retried.crossOrigin, null, "retry drops crossOrigin so the request is not a CORS request");
  assert.equal(retried.src, corsUrl, "retry keeps the same track URL");
  assert.ok(retried.loadCalls >= 1, "retry restarts resource selection");
  assert.equal(claimed.includes(retried), false, "non-CORS media is never routed into Web Audio, which would silence it");
  assert.equal(retried.playCalls, 1, "retried element plays through its own volume");
  ...
  voices.at(-1).dispatchEvent(new Event("ended")); engine.apply(state); assert.equal(sfxFinshed, 1, "finished one-shot reports its end");
  assert.equal(voices.length, 3, "finished one-shot never resurrects");
...
try {
  ...
  const mutations = [
    { name: "dedup", file: "musicBoard/room.ts", from: "if (state.recent.includes(request.requestId)) ok = true;", to: "if (false) ok = true;", run: roomTest },
    { name: "serialize", file: "musicBoard/room.ts", from: "this.queue = this.queue.then(() => this.execute(data, event.connectionId))", to: "this.queue = this.execute(data, event.connectionId)", run: roomTest },
    { name: "permission", file: "musicBoard/room.ts", from: 'if (!sender || (!automatic && sender.role !== "GM" && (!state.allowPlayers || request.op.type === "allowPlayers")))', to: 'if (!sender)', run: roomTest },
    { name: "cors-no-retry", file: "musicBoard/audio.ts", from: 'if (voice.direct) { this.status = "error"; this.changed(); return; }', to: 'this.status = "error"; this.changed(); return;', run: audioTest },
    { name: "cors-reuses-element", file: "musicBoard/audio.ts", from: "this.mount(voice, url, loop, null);", to: "voice.audio.crossOrigin = null;", run: audioTest },
  ];
  for (const mutation of mutations) { let failed = false; try { await mutation.run(mutation); } catch (error) { if (error.code !== "ERR_ASSERTION") throw error; failed = true; } assert.ok(failed, "surviving mutation " + mutation.name); }
  console.log("Music selftest PASS: actual multi-client writer serialization, identity permissions, " + mutations.length + "/" + mutations.length + " mutations, duplicate commands, handoff/late join, quota non-destruction, legacy import, audio dedup/fade cleanup, CORS fallback, Peer reconnect isolation.");
