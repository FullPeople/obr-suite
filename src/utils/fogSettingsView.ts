...
export function renderFogSettings(s: SuiteState, lang: Language, gm: boolean, authoring: boolean): string {
  ...
  return `<h3>${zh ? "玩家视野" : "Player vision"}</h3>
    ...
    <details class="fog-light-options"><summary>${zh ? "照明与高级选项" : "Lighting and advanced options"}</summary>
      ...
      <p>${zh
        ? "主光源和辅助光源需要视野授权；次光源只照亮已经可见的区域。视野归属可选：全部玩家（明确的全体授权，没有角色卡的光源也可见）、自动归属（按已绑定角色卡或创建者识别，开启共享视野后全队可用）、指定玩家、仅主持人。角色卡详情的私密设置不影响共享视野。离线成员的普通视野暂不共享，重连后恢复。"
        : "Primary and auxiliary lights require vision permission. Secondary lights illuminate already visible areas. Vision owner can be set to All players (an explicit grant that also covers lights with no card), Automatic (the bound card owner or the character creator; shared vision lets the party use such sources), one player, or GM only. Private card details stay private when vision is shared. Offline-player vision leaves the pool until they reconnect."}</p>
      ${authoring ? row("fogPlayerDoors", zh ? "玩家可以开关普通门" : "Players can operate doors", zh
        ? "允许玩家开关普通门；窗户和密门由 DM 管理。"
        : "Allow players to open and close ordinary doors. Windows and secret doors are controlled by the GM.", s.fogPlayerDoors) +
    ...
    <details class="fog-authoring-help"><summary>${zh ? "墙壁与使用帮助" : "Walls and setup help"}</summary>
      <p>${zh
        ? "FOG 图层的图形和地图迷雾编辑器保存的轮廓会生成原生墙壁，遮挡视线并阻挡单位移动。"
        : "FOG-layer drawings and map-fog-editor outlines become native walls that block vision and token movement."}</p>
      <p>${zh
        ? "窗户始终可以看穿：关闭时阻挡单位移动，打开后允许通过。由 DM 点击窗户切换开关。"
        : "Windows always let vision through. Closed windows block token movement; open windows allow passage. The GM clicks a window to open or close it."}</p>
      <p>${authoring ? (zh
        ? "迷雾工具中可画直线墙和门窗：O 普通门、I 窗户、U 密门；沿墙拖动放置，点击开关，Alt+点击或双击删除。"
        : "Use the fog tool to draw straight walls and openings: O for doors, I for windows, U for secret doors. Drag along a wall to place, click to toggle, Alt-click or double-click to remove.") : (zh
