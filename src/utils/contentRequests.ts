...
export interface JsonResource<T = any> {
  ok: boolean;
  status: number;
  /** Response validators when the host sent them. Content caches use these to
   *  decide whether a cached payload is still current. */
  etag?: string;
  lastModified?: string;
  json(): Promise<T>;
}
      ...
      const start = () => {
        ...
        void (async () => {
          try {
            ...
            finish(undefined, { ok: response.ok, status: response.status,
