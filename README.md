...
Standalone **Three-Dragon Ante 0.3.0-dev** is deployed with the supplied pack's 100 card fronts, exact Chinese printed text and four approved rule differences. Backs retain the original single-dragon vector. Central round banners and staged public scoring show card contributions, special bonuses and payouts. The six-page guide explains compulsory buying and game endings. Multiplayer power explanations still close on each viewer's click, opponents act automatically in practice, and the creator starts games without DM participation. All players must refresh and create a new table for this rule edition; previous tables and local saves remain untouched. [Install the optional card table](https://obr.dnd.center/three-dragon-ante-dev/manifest.json) and use its [release record and test checklist](docs/RELEASE_20260910_THREE_DRAGON_0_3_0.md). Music Studio remains online. Complete English XLSX downloads and character-server parser patches remain outside this release.
